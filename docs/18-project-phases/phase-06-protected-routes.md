...
- [ ] Axios interceptor automatically attempts to refresh access tokens on 401 responses
- [ ] Failed token refresh smoothly logs the user out and redirects to login
...
- [ ] Define the `matcher` array in middleware to optimize execution
- [ ] Update Axios client with a 401 response interceptor
- [ ] Implement the `refresh` API call in `authApi`
